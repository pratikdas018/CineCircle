        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-1/3 lg:w-1/4 flex-shrink-0">
            ...
            <div className="mt-4 flex flex-col gap-3">
              ...
              <button
                ...
                className={`w-full font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg hover:scale-105 border ${isInWatchlist ? "bg-gray-700 border-gray-500 text-gray-200" : "bg-gray-800 hover:bg-gray-700 text-white border-gray-600"}`}
              >
                <span>{isInWatchlist ? "❌" : "⭐"}</span> {isInWatchlist ? "Remove from Watchlist" : "Add to Watchlist"}
              </button>
              <button
                onClick={handleShare}
                className="w-full bg-gray-800 hover:bg-gray-700 text-white font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg hover:scale-105 border border-gray-600"
              >
                <span>🔗</span> Share
              </button>
            </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4 text-gray-300 mb-6">
              {movie.Released && (
                <span className="flex items-center gap-2 bg-gray-800 px-3 py-1 rounded-lg text-sm font-medium border border-gray-700">
                  📅 {movie.Released}
                </span>
              )}
              {movie.Runtime !== "N/A" && (
              ...
              {ratingStats.count > 0 && (
                <span className="flex items-center gap-2 bg-gray-800 px-3 py-1 rounded-lg text-sm font-medium border border-gray-700">
                  ...
                  <span className="text-white">{ratingStats.avg}</span>
            ...
            {movie.Ratings && (
              <div className="flex flex-wrap gap-3 mb-6">
                {movie.Ratings.map((r) => (
                  <div key={r.Source} className="flex items-center gap-2 bg-black/40 border border-gray-800 px-3 py-1.5 rounded-lg shadow-inner">
            ...
            {ratingStats.count > 0 && (
              <div className="mb-8 max-w-md bg-gray-800/50 p-4 rounded-xl border border-gray-700">
                  ...
                  {[5, 4, 3, 2, 1].map((star) => {
                    ...
                    return (
                      <div 
                        ...
                        className={`flex items-center gap-3 p-1 rounded-lg cursor-pointer transition-all ${selectedRating === star ? 'bg-gray-700 ring-1 ring-blue-500' : 'hover:bg-gray-700/50'}`}>
            ...
            <p className="text-gray-300 text-lg leading-relaxed mb-6">{movie.Plot}</p>
              ...
              <div className="mt-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {movie.Actors.split(", ").map((actor) => (
                    <div key={actor} className="bg-gray-800 px-4 py-2 rounded-full border border-gray-700 text-sm">
                      {actor}
                    </div>
                  ))}
                </div>
